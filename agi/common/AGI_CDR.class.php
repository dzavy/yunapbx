...
class AGI_CDR {
	...
	function save_cdr() {
		...
		";
		mysql_query($query) or $this->agi->verbose(mysql_error().$query);
	}
	...
	function check_transfer() {
		...
		$result  = mysql_query($query) or $this->agi->verbose(mysql_error().$query);
		$cdr_row = mysql_fetch_assoc($result);
		...
		$query = "
			...
			WHERE
				(
					channel = '".mysql_escape_string($cdr_row['dstchannel'])."'
					OR
					dstchannel = '".mysql_escape_string($cdr_row['dstchannel'])."'
				) AND dst = 's'
			LIMIT 1";
		$result  = mysql_query($query) or $this->agi->verbose(mysql_error().$query);
		if (mysql_num_rows($result) != 1) { return; }
		$cdr_s_row = mysql_fetch_assoc($result);
	...
	function push_event($event, $data) {
		...
		$query = "
			...
			SET
				...
				Data       = '$data'
		";
		mysql_query($query) or $agi->verbose(mysql_error());
	}
	...
	function monitor_getrules($caller_id, $called_id, $call_date = 0) {
		...
		$query = "
			...
			ORDER BY
				DateCreated
		";
		$result_rules = mysql_query($query) or $this->agi->verbose(mysql_error().$query);
		while ($Rule = mysql_fetch_assoc($result_rules)) {
			...
			";
			$result_ids = mysql_query($query) or $this->agi->verbose(mysql_error().$query);
			while ($id = mysql_fetch_assoc($result_ids)) {
			...
			foreach ($ActiveRule as $Active) {
				...
				$query = "
					...
					SET
						...
						Label        = '".mysql_escape_string($Active['Rule']['Label'])."',
