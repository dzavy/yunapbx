...
function get_callerid($agi, $FK_OugoingRule, $Extension) {
    ...
    if ($mysqli->numrows($result) == 1) {
        $rule = $result->fetch(PDO::FETCH_ASSOC);
        if ($rule['Type'] == 'Single') {
            if (!empty($rule['Name'])) {
...
$result = $db->query($query) or $agi->verbose($mysqli->error . $query);
while ($rule = $result->fetch(PDO::FETCH_ASSOC)) {
...
if ($rule['ProviderType'] == 'SIP') {
    ...
    $SipProvider = $result->fetch(PDO::FETCH_ASSOC);
...
// If matched rules ask us to route the number trought a IAX provider
    ...
    $IaxProvider = $result->fetch(PDO::FETCH_ASSOC);
    ...
    switch ($Extension_D['Type']) {
        case 'SipPhone' : $agi->exec_goto('Extension_SipPhone', $Extension_D['Extension'], 1);
            break;
        case 'Queue' : $agi->exec_goto('Extension_Queue', $Extension_D['Extension'], 1);
            break;
        case 'AgentLogin': $agi->exec_goto('Extension_AgentLogin', $Extension_D['Extension'], 1);
            break;
        case 'IVR' : $agi->exec_goto('Extension_IVR', $Extension_D['Extension'], 1);
            break;
        case 'Voicemail' : $agi->exec_goto('Extension_Voicemail', $Extension_D['Extension'], 1);
            break;
        case 'SimpleConf': $agi->exec_goto('Extension_SimpleConf', $Extension_D['Extension'], 1);
            break;
        case 'DialTone' : $agi->exec_goto('Extension_DialTone', $Extension_D['Extension'], 1);
            break;
        case 'Intercom' : $agi->exec_goto('Extension_Intercom', $Extension_D['Extension'], 1);
            break;
        case 'ConfCenter': $agi->exec_goto('Extension_ConfCenter', $Extension_D['Extension'], 1);
            break;
        case 'ParkingLot':
        case 'ParkingLot_Reserved':
            $agi->exec_goto('Extension_ParkingLot', $Extension_D['Extension'], 1);
            break;
        default : $agi->verbose('ERROR: Unknown extension type ' . $Extension_D['Type']);
            break;
    }
}
?>
