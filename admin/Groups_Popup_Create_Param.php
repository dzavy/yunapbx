...
function Groups_Popup_Create_Param() {
    $db = DB::getInstance();
    
    $smarty = smarty_init(dirname(__FILE__) . '/templates');
    $id_select = @$_REQUEST['id_select'];
    ...
    if (@$_REQUEST['submit'] == 'save') {
        ...
        if (count($Errors) == 0) {
			...
			<script>				
				$("#' . $id_select . '", opener.document).addOption("' . $id . '", "' . $Group['Name'] . '");
    ...
    $smarty->assign('id_select', $id_select);
    $smarty->assign('Extensions', $Extensions);
    return $smarty->fetch('Groups_Popup_Create_Param.tpl');
}
...
admin_run('Groups_Popup_Create_Param', 'AdminPopup.tpl');
?>