			...
			if (count($errors) == 0) {
				$query = "
					...
					SET
						...
						Digits          = '".mysql_real_escape_string($data['Digits'])."',
						Extension       = '".mysql_real_escape_string($data['Extension'])."',
						".(
							$data['BlockType']!='undefined'?
							"BlockType = '".mysql_real_escape_string($data['BlockType'])."',"
							:
							''
						)."
				...
				$query = "
					...
					SET
						".(
							...
							"EndNumber   = '".mysql_real_escape_string($data['EndNumber'])."',"
							:
							''
						)."
						".(
						...
						StartNumber  = '".mysql_real_escape_string($data['StartNumber'])."',
						ProviderType = '".mysql_real_escape_string($data['ProviderType'])."',
						ProviderID   = '".mysql_real_escape_string($data['ProviderID'])."'
					WHERE
