...
function CancelEdit(){
	...
	generateButton = "<button type='button' name='submit' onclick='javascript:GenerateGroups(0);' >Create Call Monitor Group</button>";
...
function ValidateMappings() {
	if ($("#listing tr").size() == 2) {
		alert('Nobody is allowed to use this feature code. You must create at least 1 intercom group.');
		return false;
	} else {
		return true;
	}
...
<form action="Extensions_FC_CallMonitor_Modify.php" method="post" onsubmit="return ValidateMappings()">
<input type="hidden" name="PK_Extension" value="{$FC_CallMonitor.PK_Extension}" />

<table id="options" class="formtable">
	<!-- Agent Login Extension -->
	<tr>
		<td>
			<div>This feature code will allow an extension to silently listen in on another extension's current call.</div>
		</td>
	</tr>

	<tr>
		<td>
			{if $FC_CallMonitor.PK_Extension}
				Feature Code Extension <strong>*{$FC_CallMonitor.Extension}</strong>
				<input type="hidden" name="PK_Extension" value="{$FC_CallMonitor.PK_Extension}" />
			{else}
			<table>
				<tr>
					<td style="width:160px;">
						...
						<br/><small>2 digits in length</small>
					</td>
					<td>
						*<input type="text" size="2" name="Extension" value="{$FC_CallMonitor.Extension}" {if $Errors.Extension}class="error"{/if} />
					</td>
				</tr>
			</table>
			{/if}
		...
		<td>
			<table>
				...
				<td>
					...
					<strong><small> that can monitor</small></strong>
					<br />
				...
				<td>
					<strong><small>Which<select name="Member_type" id="Member_type" onchange="ToggleExtGr(2)">
						<option value="Extension">Extensions</option>
						<option value="Group"    >Groups    </option>
					</select>they can monitor</small></strong>
	...
	<tr>
		<td id="zoneedit" name="zoneedit" class="template_gen_group ">
			<button  type="button" name="submit" value="generate_groups" onClick="GenerateGroups(0)">Create Monitor Group</button>
		</td>
	</tr>
</table>
