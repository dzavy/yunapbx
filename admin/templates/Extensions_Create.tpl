...
function RefreshTemplate() {
	if ($('#Type').val() != 'SipPhone' && $('#Type').val() != 'AnalogPhone' && $('#Type').val() != 'Virtual') {
		$('#FK_Template_tr').hide();
	} else {
		$('#FK_Template_tr').show();
	}
	...
	<tr>
		...
		<td>
			<select name="Type" id="Type" onchange="javascript: RefreshTemplate();">
				...
				<option {if $Type == "IVR"}selected="selected"{/if} value="IVR">IVR (Interactive Voice Response)</option>
				<option {if $Type == "GroupPickup"}selected="selected"{/if} value="GroupPickup">Group Pickup</option>
				<option {if $Type == "ConfCenter"}selected="selected"{/if} value="ConfCenter">Meet Me Conference Center</option>
				<option {if $Type == "Intercom"}selected="selected"{/if} value="Intercom">Intercom / Paging</option>
	...
	<tr id="Feature_tr">
		...
		<td>
			<select name="Feature">
				...
				<option {if $Feature == "FC_Intercom"        }selected="selected"{/if} value="FC_Intercom">Personal Intercom</option>
			</select>
		</td>
	</tr>
</table>
