...
function Extensions_FC_Intercom_Modify() {
	...
	if (@$_REQUEST['submit'] == 'save') {
		...
		if (count($Errors) == 0) {
			...
			header("Location: Extensions_List.php?msg={$msg}&hilight={$id}"); die();
		}
...
function formdata_save($data) {	
	if ($data['PK_Extension'] == "") {
		$query = "INSERT INTO Extensions(Feature, Type, Extension) VALUES(1, 'FC_Intercom', '".mysql_real_escape_string($data['Extension'])."')";
		mysql_query($query) or die(mysql_error().$query);
		$data['PK_Extension'] = mysql_insert_id();

		$query = "INSERT INTO FC_Intercom(FK_Extension) VALUES('".mysql_real_escape_string($data['PK_Extension'])."')";
		mysql_query($query) or die(mysql_error().$query);
	}
