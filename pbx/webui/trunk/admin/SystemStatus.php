...
function update_provider_statuses() {
	mysql_query("DELETE FROM SipProvider_Statuses");
	$query  = "SELECT * FROM SipProviders";
	$result = mysql_query($query) or die(mysql_error().$query);
	while ($provider = mysql_fetch_assoc($result)) {
		$Status = sip_get_status($provider);
		$query = "INSERT INTO SipProvider_Statuses SET
			FK_SipProvider = '".mysql_real_escape_string($Status['FK_SipProvider'])."',
			Latency        = '".mysql_real_escape_string($Status['Latency'])."',
			Status         = '".mysql_real_escape_string($Status['Status'])."'
		";
		mysql_query($query) or die(mysql_error().$query);
	}

	/////////////////////
	mysql_query("DELETE FROM IaxProvider_Statuses");
	$query  = "SELECT * FROM IaxProviders";
	$result = mysql_query($query) or die(mysql_error().$query);
	while ($provider = mysql_fetch_assoc($result)) {
		$Status = iax_get_status($provider);
		$query = "INSERT INTO IaxProvider_Statuses SET
			FK_IaxProvider = '".mysql_real_escape_string($Status['FK_IaxProvider'])."',
			Latency        = '".mysql_real_escape_string($Status['Latency'])."',
			Status         = '".mysql_real_escape_string($Status['Status'])."'
		";
		mysql_query($query) or die(mysql_error().$query);
...
function update_Ext_SipPhones_Statuses() {
	...
	while ($row = mysql_fetch_assoc($result)) {
		...
		if (strpos($status['Status'], "OK") === false) {
			$query2  = "SELECT * FROM Ext_SipPhones_Status WHERE Extension = '".mysql_real_escape_string($Extension)."' AND Status IN ('OK','TIMEOUT') LIMIT 1";
			if (mysql_numrows(mysql_query($query2)) == 1) {
				$Status = "TIMEOUT";
			} else {
				$Status = "UNREACHABLE";
			}
		...
		if ($Status != "TIMEOUT") {
			...
			";
			mysql_query("DELETE FROM Ext_SipPhones_Status WHERE Extension = $Extension LIMIT 1");
			mysql_query($query) or die(mysql_error().$query);
		} else {
			mysql_query("UPDATE Ext_SipPhones_Status SET Status='TIMEOUT' WHERE Extension = $Extension LIMIT 1");
		}

		mysql_query("DELETE FROM Ext_SipPhones_Status WHERE Extension NOT IN (SELECT Extension FROM Extensions WHERE Type = 'SipPhone')");
	}
}
