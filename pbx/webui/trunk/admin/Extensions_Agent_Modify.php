...
function Extensions_Agent_Modify() {
	...
	if (@$_REQUEST['submit'] == 'save') {
		...
		if (count($Errors) == 0) {
			...
			asterisk_Reload();
			header("Location: Extensions_List.php?msg=MODIFY_AGENT_EXTENSION&hilight={$id}"); die();
		}
...
function formdata_save($data) {
	if ($data['PK_Extension'] == "") {
		$query = "INSERT INTO Extensions(Extension,Type) VALUES('".mysql_real_escape_string($data['Extension'])."', 'Agent')";
		mysql_query($query) or die(mysql_error().$query);
		$data['PK_Extension'] = mysql_insert_id();

		$query = "INSERT INTO Ext_Agent(PK_Extension) VALUES('".mysql_real_escape_string($data['PK_Extension'])."')";
		mysql_query($query) or die(mysql_error().$query);
	}
