...
function vm_folders($extension) {
	...
	if (preg_match('/\.\.\//', $extension)) { return $Folders; }
		
	// If the voicemail does not exist return
	if (!is_dir($vm_path)) return $Folders;
...
function vm_files($extension, $folder) {
	...
	// Prevent path traversal
	if (preg_match('/\.\.\//', $folder))    { return $messages; }
	if (preg_match('/\.\.\//', $extension)) { return $messages; }

	// If the voicemail does not exist return null filelist
	if (!is_dir($vm_path)) return $messages;
...
function vm_getfile($extension, $folder, $file) {
	...
	// Prevent path traversal
	if (preg_match('/\.\.\//', $extension)) { return false; }
	if (preg_match('/\.\.\//', $folder))    { return false; }
...
function vm_movfile($extension, $src_folder, $dst_folder, $src_file) {
	...
	// Prevent path traversal
	if (preg_match('/\.\.\//', $extension))  { return false; }
	if (preg_match('/\.\.\//', $src_folder)) { return false; }
	if (preg_match('/\.\.\//', $dst_folder)) { return false; }
	if (preg_match('/\.\.\//', $src_file))   { return false; }
